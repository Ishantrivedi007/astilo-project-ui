...
const PosterCard = ({ item, basePath }: { item: MediaItem; basePath: string }) => (
  <Link
...
const MovieRow = ({ label, emoji, items, isLoading, basePath = "/movies" }: MovieRowProps) => {
    ...
    <section className="mt-10">
      ...
      >
        {isLoading
          ...
          : items!.map((item) => (
              <PosterCard key={`${item.kind}-${item.id}`} item={item} basePath={basePath} />
            ))}
      </div>
    </section>
  );
